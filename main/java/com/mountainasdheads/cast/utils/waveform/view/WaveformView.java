package com.mountainasdheads.cast.utils.waveform.view;
...
import com.mountainasdheads.cast.utils.waveform.Segment;
...
public class WaveformView extends View {
    ...
    public WaveformView(Context context, AttributeSet attrs) {
        ...
        mPlaybackLinePaint.setStrokeWidth(2);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mSoundFile == null)
            return;
        yGlobal = Math.round(mDensity * 50);
        int measuredWidth = getMeasuredWidth();
        int measuredHeight = getMeasuredHeight()-yGlobal;
        int start = mOffset;
        ...
        yEnd = Math.round(getMeasuredHeight()-mDensity*24);
        yStart = Math.round(mDensity*24);
        ...
        while (i < width) {
            ...
            if (integerSecsNew != integerSecs) {
                ...
                if (!onlyEveryFiveSecs || 0 == (integerSecs % 5)) {
                    canvas.drawLine(i + 1, mDensity * 14, i + 1, mDensity*24, mGridPaint);
                }
                if (!onlyEveryFiveSecs || 0 == (integerSecs % 1)) {
                    canvas.drawLine(i + 1, mDensity * 18, i + 1, mDensity*24, mGridPaint);
                }
            }
        ...
        for (i = width; i < measuredWidth; i++) {
            drawWaveformLine(canvas, i, yStart, yEnd, mUnselectedBkgndLinePaint);
        }
        ...
        paint.setColor(getResources().getColor(R.color.blue_stroke));
        Rect r = new Rect(0, Math.round(mDensity*24), getMeasuredWidth(), yEnd);
    ...
    protected void drawWaveform(final Canvas canvas, final int i, final int start, final int measuredHeight, final int ctr, final Paint paint) {
        if (i + start < mSelectionStart || i + start >= mSelectionEnd) {
            drawWaveformLine(canvas, i, yStart, yEnd,
        ...
        if (i + start == mPlaybackPos) {
            canvas.drawLine(i, 0+yStart, i, yEnd, mPlaybackLinePaint);
        }
    }
